...
def train(loader, model, crit, opt, epoch, num_classes):
    ...
    for i, (input_tensor, target) in enumerate(loader):
        ...
        _, _, pseudo_label, _, _, _, pred_att, mask = model(X)

        loss_cluster_pred = crit(pseudo_label, target_var)
        loss_att = torch.sum(pred_att*target_attention_var)
        loss_L21 = torch.sqrt(torch.sum(mask ** 2))  * 0.001

        loss = loss_cluster_pred + loss_att + loss_L21
...
for DATASET in DATASETS:
    ...
    model = SOFT_FIRST(num_features, NUM_CLASSES, USE_CUDA=USE_CUDA)
    if USE_CUDA:
        model = model.cuda()
    ...
    for epoch in range(EPOCHS):
        flag, _, _, A_att, _, _, _, mask = model(X)
    ...
    sim_file = 'results/' + DATASET + '_result.csv'
